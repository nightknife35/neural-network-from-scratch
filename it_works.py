...
model.layers[1].weights = weights[0].T
model.layers[1].biases = weights[1].T
model.layers[2].weights = weights[2].T
model.layers[2].biases = weights[3].T
model.layers[3].weights = weights[4].T
model.layers[3].biases = weights[5].T
...
for i in range(num_images):
    ...
    ax.set_title(f'{y[i]} vs {y_hat[i]}: {wrong[i]}')
    ax.axis('off')
