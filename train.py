...
class NeuralNetwork:
    ...
    def backwards(self, x, y, lr):# lr = lr
        ...
        weight_adjustment = [] # [3x3, 2x2, 3x3] changes we wish to make to these weights
        bias_adjustment = []   # [3x1, 2x1, 3x1]                                        |
        self.all_weights = []  # [3x3, 2x2, 3x3] all weights taht ARE in the network  <-|
        self.all_biases = []
    ...
    def train(self, path_to_images, path_to_labels, lr, epochs, batch):
        ...
        for epoch in range(epochs):
            correct = 0
            for i in range(int(len(os.listdir(path_to_images))/40)): # all of the images 60.000
                ...
                if self.predict(image) == label:
                    correct+=1
            correct/=(len(os.listdir(path_to_images))/40)
...
# training
model.train(path_to_images, path_to_labels, lr= 0.01, epochs = 10, batch = 32)



weights_list = []
weights_list.append(model.layers[1].weights)
weights_list.append(model.layers[1].bias)
weights_list.append(model.layers[2].weights)
weights_list.append(model.layers[2].bias)
weights_list.append(model.layers[3].weights)
weights_list.append(model.layers[3].bias)
weights = [w.tolist() for w in weights_list]


with open('mine.json', 'w') as json_file:
